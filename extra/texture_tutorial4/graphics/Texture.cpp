...
bool CTexture::LoadFile(LPCTSTR lpszPathName)
{
   ...
   if(!file)
   {
      AfxMessageBox("Unsupported file type read");
      return false;
   }
...
bool CTexture::ReadDIBFile(istream &file)
{
   ...
   if(!file)
   {
      AfxMessageBox("Unsupported image file type");
      return false;
   }

	if (bmfHeader.bfType != DIB_HEADER_MARKER)
   {
      AfxMessageBox("Note a BMP file");
      return false;
   }
   ...
   {
      delete (BYTE *)pBMI;
      AfxMessageBox("Premature end of file in image file");
      return false;
   }
